...
echo "=== TESTING SYNC 2 WITH ACCURATE COHORTS & 2026 LOCK ===\n\n";
...
// Semesters belonging exclusively to 2025/2026
...
echo "2025/2026 Semesters: " . $semesters2025->pluck('id')->join(', ') . " (" . $semesters2025->pluck('name')->join(', ') . ")\n";

// Strictly Excluded: New students enrolled for 2026/2027 onwards
$new2026StudentIds = DB::table('students')
    ->where('created_at', '>=', '2026-07-01')
    ->orWhere('nis', 'like', 'TEST%')
    ->pluck('id')
    ->toArray();

echo "Newly enrolled 2026/2027 students (EXCLUDED from 2025/2026): " . count($new2026StudentIds) . " students\n\n";
...
$s9aCohort = DB::table('students')
    ...
    ->whereNotIn('id', $new2026StudentIds)
    ->pluck('id');
...
if (Schema::hasTable('subject_attendances')) {
    $saRows = DB::table('subject_attendances as sa')
        ...
        ->whereNotIn('sa.student_id', $new2026StudentIds)
...
// 3. Siswa Kelas 2025/2026 (school_classes ID 1..5)
$directStudents = DB::table('students as st')
    ...
    ->whereNotIn('st.id', $new2026StudentIds)
...
// 4. Siswa 2024xxxx -> Kelas 8 (8A, 8B, 8C, 8D)
...
// 5. Siswa 2025/2026 Grade 7 (NIS 011/012/312 atau created in 2025)
...
echo "Kelas 9A student count: " . count($byClass['Kelas 9A'] ?? []) . " students (EXPECTED: 29)\n";
echo "Are any 2026/2027 new students included? " . (count(array_intersect(array_keys($studentClassMap), $new2026StudentIds)) > 0 ? "YES (ERROR!)" : "NO (100% CLEAN!)") . "\n";
