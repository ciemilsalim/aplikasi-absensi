...
@php
    ...
    if (!$parentUrl) {
        if ($count > 1) {
            // Target parent URL is the item right before the active/current item
    ...
    <div {{ $attributes->merge(['class' => 'flex items-center gap-2 flex-wrap mb-1']) }}>
        @if ($showBack && $parentUrl)
            <a href="{{ ($parentUrl !== '#' && !empty($parentUrl)) ? $parentUrl : 'javascript:void(0);' }}" 
               ...
               class="inline-flex items-center gap-1 px-2.5 py-1 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-750 text-slate-700 dark:text-slate-300 font-bold text-xs border border-slate-200/80 dark:border-slate-700 transition-all shadow-2xs group shrink-0"
               title="Kembali ke {{ $parentTitle }}">
                <span class="material-icons text-sm text-slate-500 group-hover:text-sky-600 dark:group-hover:text-sky-400 group-hover:-translate-x-0.5 transition-transform">arrow_back</span>
                <span>Kembali</span>
            </a>
        @endif
