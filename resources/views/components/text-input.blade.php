...
@if ($type === 'password')
    <div x-data="{ showPassword: false }" class="relative">
        <input {{ $disabled ? 'disabled' : '' }} 
               ...
               {!! $attributes->merge(['class' => 'w-full pr-11 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 rounded-xl text-sm shadow-2xs transition-all disabled:opacity-50 disabled:bg-slate-50 dark:disabled:bg-slate-800']) !!}>
...
@else
    ...
    <input {{ $disabled ? 'disabled' : '' }} type="{{ $type }}" {!! $attributes->merge(['class' => 'w-full border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 rounded-xl text-sm shadow-2xs transition-all disabled:opacity-50 disabled:bg-slate-50 dark:disabled:bg-slate-800']) !!}>
@endif
