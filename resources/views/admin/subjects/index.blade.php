...
function sortable_link($title, $column, $sortBy, $sortDirection) {
    ...
    return '<a href="' . route('admin.subjects.index', $queryParams) . '" class="flex items-center gap-2">' . $title . ' <span class="text-sky-500">' . $arrow . '</span></a>';
}
@endphp
            ...
            <div class="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="flex flex-col sm:flex-row gap-4 justify-between items-center mb-6">
                        ...
                        <a href="{{ route('admin.subjects.create') }}" class="inline-flex items-center px-4 py-2 bg-sky-600 text-white rounded-md hover:bg-sky-700 text-sm font-medium">
                    ...
                    <form method="GET" action="{{ route('admin.subjects.index') }}" class="mb-6">
                        <div class="flex flex-col sm:flex-row gap-4">
                            <input type="hidden" name="sort_by" value="{{ request('sort_by') }}">
                            <input type="hidden" name="sort_direction" value="{{ request('sort_direction') }}">
                            
                            <div class="relative flex-grow">
                                <x-text-input type="text" name="search" placeholder="Cari nama atau kode mata pelajaran..." value="{{ request('search') }}" class="w-full pl-10"/>
                    ...
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-slate-700 dark:text-gray-400">
                                <tr>
                                    <th scope="col" class="px-6 py-3">{!! sortable_link('Nama Mata Pelajaran', 'name', $sortBy, $sortDirection) !!}</th>
                                    <th scope="col" class="px-6 py-3">{!! sortable_link('Kode', 'code', $sortBy, $sortDirection) !!}</th>
                            ...
                            <tbody>
                                ...
                                <tr class="bg-white border-b dark:bg-slate-800 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-600">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-4">
                                            ...
                                            <button type="button" 
                                                    @click="showConfirmModal = true; deleteUrl = '{{ route('admin.subjects.destroy', $subject) }}'"
                                ...
                                @empty
                                <tr><td colspan="4" class="px-6 py-4 text-center">Tidak ada data mata pelajaran.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                    <div class="mt-4">{{ $subjects->appends(request()->query())->links() }}</div>
                </div>
            </div>
        </div>
        ...
        <div x-show="showConfirmModal" x-transition class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm" style="display: none;">
            <div @click.away="showConfirmModal = false" x-show="showConfirmModal" x-transition class="w-full max-w-md p-6 bg-white dark:bg-slate-800 rounded-2xl shadow-xl">
                <div class="text-center">
                    ...
                    <h3 class="mt-5 text-lg font-medium text-gray-900 dark:text-white">Hapus Mata Pelajaran?</h3>
                    <div class="mt-2">
