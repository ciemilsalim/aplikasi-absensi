                            ...
                            <tbody>
                                ...
                                <tr class="bg-white border-b dark:bg-slate-800 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-600"
                                    ...
                                    <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">
                                        ...
                                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ $request->student->schoolClass->name ?? '' }}</p>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        @if ($request->status == 'pending')
                                            <div class="flex items-center gap-2" x-data="{ showRejectForm: false }">
                                                <form action="{{ route('admin.leave_requests.approve', $request) }}" method="POST" onsubmit="return confirm('Anda yakin ingin menyetujui pengajuan ini?');">
                                                    @csrf
                                                ...
                                                <!-- Form Tolak dalam Modal/Dropdown -->
                                ...
                                @empty
                                <tr><td colspan="5" class="px-6 py-4 text-center">Tidak ada pengajuan izin/sakit yang masuk.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
