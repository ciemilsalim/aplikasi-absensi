        ...
        <ul role="list" class="flex flex-1 flex-col gap-y-7">
                        ...
                        @endif
                         @if(auth()->user()->role === 'teacher')
                            ...
                            @if(Auth::user()->teacher && Auth::user()->teacher->homeroomClass)
                                 {{-- PERBAIKAN: Menambahkan badge notifikasi untuk guru --}}
                                 <li>
                                     <a href="{{ route('teacher.leave_requests.index') }}" class="{{ request()->routeIs('teacher.leave_requests.*') ? 'bg-slate-100 dark:bg-slate-700 text-sky-600 dark:text-white' : 'text-slate-700 dark:text-slate-300 hover:text-sky-600 hover:bg-slate-50 dark:hover:bg-slate-700' }} group flex items-center justify-between gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold">
                                         ...
                                         @if(isset($teacherPendingLeaveRequestsCount) && $teacherPendingLeaveRequestsCount > 0)
                                            <span class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-red-500 text-white text-xs font-medium">{{ $teacherPendingLeaveRequestsCount }}</span>
                                         @endif
                                     </a>
                                 </li>
                            @endif
            ...
            <li>
                ...
                <ul role="list" class="-mx-2 mt-2 space-y-1">
                    ...
                    <li>
                        <a href="{{ route('admin.announcements.index') }}" class="{{ request()->routeIs('admin.announcements.*') ? 'bg-slate-100 dark:bg-slate-700 text-sky-600 dark:text-white' : 'text-slate-700 dark:text-slate-300 hover:text-sky-600 hover:bg-slate-50 dark:hover:bg-slate-700' }} group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold">
                     ...
                     <li><a href="{{ route('admin.backup.index') }}" class="{{ request()->routeIs('admin.backup.*') ? 'bg-slate-100 dark:bg-slate-700 text-sky-600 dark:text-white' : 'text-slate-700 dark:text-slate-300 hover:text-sky-600 hover:bg-slate-50 dark:hover:bg-slate-700' }} group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6 shrink-0"><path stroke-linecap="round" stroke-linejoin="round" d="M20.25 6.375c0 2.278-3.694 4.125-8.25 4.125S3.75 8.653 3.75 6.375m16.5 0c0-2.278-3.694-4.125-8.25-4.125S3.75 4.097 3.75 6.375m16.5 0v11.25c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125V6.375m16.5 0v3.75m-16.5-3.75v3.75m16.5 0v3.75C20.25 16.153 16.556 18 12 18s-8.25-1.847-8.25-4.125v-3.75m16.5 0c-4.556 0-8.25 1.847-8.25 4.125v3.75m-8.25-4.125c0 2.278 3.694 4.125 8.25 4.125s8.25-1.847 8.25-4.125" /></svg>Backup</a></li>
                </ul>
            </li>
            <li class="mt-auto">
                 <a href="{{ route('admin.settings.identity') }}" class="{{ request()->routeIs('admin.settings.*') ? 'bg-slate-100 dark:bg-slate-700 text-sky-600 dark:text-white' : 'text-slate-700 dark:text-slate-300 hover:text-sky-600 hover:bg-slate-50 dark:hover:bg-slate-700' }} group -mx-2 flex gap-x-3 rounded-md p-2 text-sm font-semibold leading-6">
