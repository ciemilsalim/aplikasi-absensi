...
<!-- DAFTAR SISWA UNTUK DIKELOLA (LEBAR PENUH 100% DI DESKTOP, KARTU DI MOBILE) -->
<div x-data="{ search: '', statusFilter: 'all' }" class="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg my-6">
    <div class="p-4 sm:p-6 text-gray-900 dark:text-gray-100">
        ...
        <!-- TAMPILAN DESKTOP: TABEL LEBAR PENUH 100% (≥ sm breakpoint) -->
        <div class="hidden sm:block overflow-hidden rounded-xl border border-gray-200 dark:border-slate-700">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-slate-700 dark:text-gray-300 border-b border-gray-200 dark:border-slate-700">
                    <tr>
                        <th scope="col" class="px-5 py-3.5 font-semibold">Nama Siswa</th>
                        <th scope="col" class="px-4 py-3.5 text-center font-semibold w-36">Jam Masuk</th>
                        <th scope="col" class="px-4 py-3.5 text-center font-semibold w-36">Jam Pulang</th>
                        <th scope="col" class="px-4 py-3.5 text-center font-semibold w-40">Status</th>
                        <th scope="col" class="px-4 py-3.5 text-center font-semibold w-52">Aksi Absensi Cepat</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200 dark:divide-slate-700">
                        ...
                        <tr class="bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors"
                            ...
                            <td class="px-5 py-3.5 font-medium text-gray-900 dark:text-white">
                                <div class="flex items-center gap-3">
                                    <div class="relative group flex-shrink-0">
                                        <img class="h-10 w-10 rounded-full object-cover border-2 border-slate-200 dark:border-slate-700 shadow-sm" 
                                             src="{{ $student->photo_url }}" 
                                             onerror="this.onerror=null;this.src='https://ui-avatars.com/api/?name={{ urlencode($student->name) }}&color=7F9CF5&background=EBF4FF';"
                                             alt="{{ $student->name }}">
                                    ...
                                    <div>
                                        <p class="font-semibold text-slate-800 dark:text-white text-sm">{{ $student->name }}</p>
                                        <p class="text-xs text-gray-500 dark:text-gray-400">NIS: {{ $student->nis ?? '-' }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-3.5 text-center">
                                @if($attendance && $attendance->attendance_time && !in_array($attendance->status, ['izin', 'sakit', 'alpa']))
                                    <span class="bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300 text-xs font-semibold px-2.5 py-1 rounded-md border border-emerald-200 dark:border-emerald-800/50">{{ \Carbon\Carbon::parse($attendance->attendance_time)->format('H:i:s') }}</span>
                                @else
                            ...
                            <td class="px-4 py-3.5 text-center">
                                @if($attendance && $attendance->checkout_time)
                                    <span class="bg-indigo-50 text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-300 text-xs font-semibold px-2.5 py-1 rounded-md border border-indigo-200 dark:border-indigo-800/50">{{ \Carbon\Carbon::parse($attendance->checkout_time)->format('H:i:s') }}</span>
                                @else
                                    <span class="text-gray-400 text-xs">-</span>
                                @endif
                            </td>
                            <td class="px-4 py-3.5 text-center">
                                @if($attendance)
                                    @if ($attendance->status === 'tepat_waktu')<span class="bg-emerald-100 text-emerald-800 text-xs font-semibold px-3 py-1 rounded-full dark:bg-emerald-900/60 dark:text-emerald-300">Tepat Waktu</span>
                                    @elseif ($attendance->status === 'terlambat')<span class="bg-amber-100 text-amber-800 text-xs font-semibold px-3 py-1 rounded-full dark:bg-amber-900/60 dark:text-amber-300">Terlambat</span>
                                    @elseif ($attendance->status === 'izin')<span class="bg-purple-100 text-purple-800 text-xs font-semibold px-3 py-1 rounded-full dark:bg-purple-900/60 dark:text-purple-300">Izin</span>
                                    @elseif ($attendance->status === 'sakit')<span class="bg-yellow-100 text-yellow-800 text-xs font-semibold px-3 py-1 rounded-full dark:bg-yellow-900/60 dark:text-yellow-300">Sakit</span>
                                    @elseif ($attendance->status === 'alpa')<span class="bg-rose-100 text-rose-800 text-xs font-semibold px-3 py-1 rounded-full dark:bg-rose-900/60 dark:text-rose-300">Alpa</span>
                                    @elseif ($attendance->status === 'izin_keluar')<span class="bg-blue-100 text-blue-800 text-xs font-semibold px-3 py-1 rounded-full dark:bg-blue-900/60 dark:text-blue-300">Izin Keluar</span>
                                    @endif
                                @else
                                    <span class="bg-slate-100 text-slate-600 text-xs font-medium px-3 py-1 rounded-full dark:bg-slate-700 dark:text-slate-300">Belum Hadir</span>
                                @endif
                            </td>
                            <td class="px-4 py-3.5 text-center">
                                @if(!$attendance)
                                    <div class="flex items-center justify-center gap-1.5">
                                        <form action="{{ route('teacher.mark.attendance') }}" method="POST">
                                            ...
                                            <input type="hidden" name="status" value="sakit">
                                            <input type="hidden" name="date" value="{{ now()->format('Y-m-d') }}">
                                            <button type="submit" title="Tandai Sakit" class="px-2.5 py-1 text-xs font-semibold text-yellow-800 bg-yellow-100 hover:bg-yellow-200 dark:bg-yellow-900/60 dark:text-yellow-200 dark:hover:bg-yellow-800 rounded-md transition-colors">Sakit</button>
                                        </form>
                                        <form action="{{ route('teacher.mark.attendance') }}" method="POST">
                                            @csrf
                                            <input type="hidden" name="student_id" value="{{ $student->id }}">
                                            <input type="hidden" name="status" value="izin">
                                            <input type="hidden" name="date" value="{{ now()->format('Y-m-d') }}">
                                            <button type="submit" title="Tandai Izin" class="px-2.5 py-1 text-xs font-semibold text-purple-800 bg-purple-100 hover:bg-purple-200 dark:bg-purple-900/60 dark:text-purple-200 dark:hover:bg-purple-800 rounded-md transition-colors">Izin</button>
                                        </form>
                                        <form action="{{ route('teacher.mark.attendance') }}" method="POST">
                                            @csrf
                                            <input type="hidden" name="student_id" value="{{ $student->id }}">
                                            <input type="hidden" name="status" value="alpa">
                                            <input type="hidden" name="date" value="{{ now()->format('Y-m-d') }}">
                                            <button type="submit" title="Tandai Alpa" class="px-2.5 py-1 text-xs font-semibold text-rose-800 bg-rose-100 hover:bg-rose-200 dark:bg-rose-900/60 dark:text-rose-200 dark:hover:bg-rose-800 rounded-md transition-colors">Alpa</button>
                                        </form>
                                    </div>
                                @else
                                    <form action="{{ route('teacher.mark.attendance') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin mereset/menghapus status kehadiran siswa ini?')">
                                        ...
                                        <input type="hidden" name="status" value="hapus">
                                        <input type="hidden" name="date" value="{{ now()->format('Y-m-d') }}">
                                        <button type="submit" title="Reset Status Kehadiran" class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium text-slate-600 dark:text-slate-300 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 rounded-md transition-colors">
                    ...
                    @empty
                        <tr class="bg-white dark:bg-slate-800">
                            <td colspan="5" class="px-6 py-6 text-center text-gray-500 dark:text-gray-400">Tidak ada siswa di kelas ini.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
<!-- SEKSI BAWAH: GRAFIK & PANEL PENGINGAT (GRID 3 KOLOM) -->
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <!-- Kolom Kiri: Grafik Tren Kehadiran Mingguan -->
